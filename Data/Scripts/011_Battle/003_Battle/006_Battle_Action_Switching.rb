class PokeBattle_Battle
  ...
  def pbOnActiveOne(battler)
    ...
    if battler.pbOwnSide.effects[PBEffects::StealthRock] && battler.takesIndirectDamage? &&
       GameData::Type.exists?(:ROCK) && !battler.affectedByEntryHazards? 
    ...
    if battler.pbOwnSide.effects[PBEffects::Spikes]>0 && battler.takesIndirectDamage? &&
       !battler.airborne? && !battler.affectedByEntryHazards?
    ...
    if battler.pbOwnSide.effects[PBEffects::ToxicSpikes]>0 && !battler.fainted? &&
      ...
      elsif battler.pbCanPoison?(nil,false) && !battler.affectedByEntryHazards?
